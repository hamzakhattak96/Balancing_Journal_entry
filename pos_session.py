...
class PosSession(models.Model):
    ...
    def _prepare_balancing_line_vals(self, imbalance_amount, move):
        ...
        # raise UserError(str(account))
        #zeeshan
        partial_vals = {
            'name': _('From invoiced orders'),
            'account_id': 6,
            'move_id': move.id,
            'partner_id': False,
        }
    ...
    def _create_account_move(self):
        ...
        """
        #Hamza
        # raise UserError(str(self.payment_ids))
        ...
        if account_move:
            ...
            # raise UserError(str(len(data['order_account_move_receivable_lines'])))
            data = self._create_non_reconciliable_move_lines(data)
            data = self._create_cash_statement_lines_and_cash_move_lines(data)
            # raise UserError(str(data = self._create_invoice_receivable_lines(data)))
