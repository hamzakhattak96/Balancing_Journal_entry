...
class AccountMove(models.Model):
    ...
    def _move_autocomplete_invoice_lines_create(self, vals_list):
        ...
        '''
        new_vals_list = []
        # raise UserError(str(vals_list[0]['invoice_line_ids'][0][0]['account_id.name']))
        # raise Warning('HamzaKhattak')
        # raise Warning(str(vals_list[0]['invoice_line_ids'][0][2]['name']))
        for vals in vals_list:
            # if vals_list[0]['journal_id'] == 7:
            #     raise UserError(str(vals_list[0][7]['account_id.name']))
            # # if vals_list[0]['invoice_line_ids'][0][2]['name'] != 'Credit':
            ...
            # if vals['invoice_line_ids'][0][2]['name'] == 'Credit':
            # raise (str(vals['invoice_line_ids'][0][2]['name']))
            vals.pop('invoice_line_ids', None)

            move = self_ctx.new(vals)
            new_vals_list.append(move._move_autocomplete_invoice_lines_values())
            # raise UserError(str(new_vals_list[0][2]['account_id.name']))
            # raise UserError((new_vals_list['line_ids'][0]['name']))
    ...
    def _move_autocomplete_invoice_lines_write(self, vals):
        ...
        # raise UserError(str(vals['invoice_line_ids']))
        # raise UserError('Hello WOrfld')
        # raise UserError(str(vals['invoice_line_ids'][0][1]))
        # HamzaKhattak
        # if vals['invoice_line_ids'][0][2]['account_'] != 'Credit':
        if vals['invoice_line_ids'][0][2]['name'] != 'Credit':
    ...
    def _create_writeoff(self, writeoff_vals):
        ...
        for journal_id, lines in writeoff_dict.items():
            ...
            for line in writeoff_lines:
                raise UserError(str(writeoff_lines))
            
            # Create the move
            # HamzaKhattak
                writeoff_move = self.env['account.move'].create({
                    'journal_id': 'HamzaKhattak',
                    # 'journal_id': journal_id,
                    'date': date,
                    'state': 'draft',
                    # if line.account_id.name != '101103 Credit Receivable' for line in writeoff_lines,
                    # 'line_ids': [(0, 0, line),  
                    'line_ids': [(0, 0, line) for line in writeoff_lines if line.account_id.name != '101103 Credit Receivable'],
                })
